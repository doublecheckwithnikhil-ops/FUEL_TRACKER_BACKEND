...
async function main() {
    ...
    const roleNames = ["driver", "admin", "super_admin"];
    const roles = await Promise.all(roleNames.map(async (name) => prisma.role.upsert({
        where: { name },
        update: {},
        create: { name },
    })));
    ...
    const passwordHash = await bcryptjs_1.default.hash("V2@123", 10);
    if (!existing) {
